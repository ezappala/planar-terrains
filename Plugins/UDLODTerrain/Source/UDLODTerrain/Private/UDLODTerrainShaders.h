...
class FUDLOD_RootTileCS : public FGlobalShader {
    ...
    BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FUDLODTileGpu>, root_working_tiles)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, root_working_count)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, root_final_count)
    END_SHADER_PARAMETER_STRUCT()
};

BEGIN_UNIFORM_BUFFER_STRUCT(UDLOD_DispatchCB,)
...
class FUDLOD_MakeDispatchArgsCS : public FGlobalShader {
    ...
    BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
        SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, dispatch_in_count)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, dispatch_out_args)
        SHADER_PARAMETER_RDG_UNIFORM_BUFFER(UDLOD_DispatchCB, DispatchCB)
    END_SHADER_PARAMETER_STRUCT()
};

BEGIN_UNIFORM_BUFFER_STRUCT(UDLOD_RefineCB,)
...
class FUDLOD_RefineCS : public FGlobalShader {
    ...
    BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
        SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUDLODTileGpu>, in_tiles)
        SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, in_count)

        SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FUDLODTileGpu>, out_tiles)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, out_count)

        SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FUDLODTileGpu>, final_tiles)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, final_count)

        SHADER_PARAMETER_TEXTURE(Texture2D, height_texture)
        SHADER_PARAMETER_SAMPLER(SamplerState, height_sampler)

        SHADER_PARAMETER_RDG_UNIFORM_BUFFER(UDLOD_RefineCB, RefineCB)
    END_SHADER_PARAMETER_STRUCT()
};

BEGIN_UNIFORM_BUFFER_STRUCT(UDLOD_DrawArgsCB,)
...
class FUDLOD_DrawArgsCS : public FGlobalShader {
    ...
    BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
        SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, draw_in_final_count)
        SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, draw_out_args)
        SHADER_PARAMETER_RDG_UNIFORM_BUFFER(UDLOD_DrawArgsCB, DrawArgsCB)
    END_SHADER_PARAMETER_STRUCT()
};

BEGIN_UNIFORM_BUFFER_STRUCT(UDLOD_VSParamsCB,)
    SHADER_PARAMETER(FVector2f, vs_world_min_xy_ws)
    SHADER_PARAMETER(FVector2f, vs_world_size_xy_ws)
    SHADER_PARAMETER(FVector2f, vs_height_minmax_ws)
    SHADER_PARAMETER(uint32, vs_grid_resolution)
END_UNIFORM_BUFFER_STRUCT()

BEGIN_SHADER_PARAMETER_STRUCT(FUDLOD_TerrainPassParameters,)
    SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, view)
    SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUDLODTileGpu>, vs_final_tiles)
    SHADER_PARAMETER_TEXTURE(Texture2D, vs_height_texture)
    SHADER_PARAMETER_SAMPLER(SamplerState, vs_height_sampler)

    SHADER_PARAMETER_RDG_UNIFORM_BUFFER(UDLOD_VSParamsCB, VSParamsCB)
