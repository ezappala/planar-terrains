 ...
 UPROPERTY(VisibleAnywhere, Category = "UDLOD")
    ...
    static TOptional<FTerrainConfig> from_json(const TSharedPtr<FJsonObject>& json_object) {
        ...
        for (const auto& [label, value] : attachments_json->Values) {
            ...
            FAttachmentConfig attachment_config{};
            attachment_config.texture_size = config_json->GetNumberField(
                FString{"texture_size"});
            attachment_config.border_size = config_json->GetNumberField(
                FString{"border_size"});
            attachment_config.mip_level_count = config_json->GetNumberField(
                FString{"mip_level_count"});
            attachment_config.mask = config_json->GetBoolField(
                FString{"mask"});
    ...
    void save_file(const FString& file_path) const {
        ...
        for (const auto& [label, config] : attachments) {
            ...
            config_json->SetNumberField(
                FString{"format"},
                static_cast<int32>(config.format));
            attachments_json->Values.Add(
                label,
                MakeShared<FJsonValueObject>(config_json));
        }
        ...
        json_object->Values.Add(
            FString{"tiles"},
            MakeShared<FJsonValueArray>(tiles_json));

        FString encoded;
        const TSharedRef<TJsonWriter<>> json_writer = TJsonWriterFactory
            <>::Create(&encoded);
        if (FJsonSerializer::Serialize(
            json_object.ToSharedRef(),
            json_writer)) { FFileHelper::SaveStringToFile(encoded, *file_path); } else {
            UE_LOG(
                LogTemp,
                Error,
