...
void TransposeTransforms(FMatrix2x4* DstTransforms, const FMatrix44f* SrcTransforms, int64 Count) {
    ...
    for (; TransformIndex + 2 <= Count; TransformIndex += 2) {
        ...
        FMatrix2x4& DstTransformA = DstTransforms[TransformIndex + 0];
        FMatrix2x4& DstTransformB = DstTransforms[TransformIndex + 1];
    ...
    if (TransformIndex < Count) { TransposeTransform(DstTransforms[TransformIndex], SrcTransforms[TransformIndex]); }
#else
    for (int64 TransformIndex = 0; TransformIndex < Count; ++TransformIndex) {
        const FMatrix44f& SrcTransform = SrcTransforms[TransformIndex];
        FMatrix2x4& DstTransform = DstTransforms[TransformIndex];
