param(
    ...
    [string]$TestFilter = "UDLODPreprocessor",
    [switch]$Build,
    [switch]$Coverage
)
...
$runnerCmd = Join-Path $PSScriptRoot "Run-UDLODPreprocessor-Tests.cmd"
...
if ($Coverage) {
    $coveragePath = Join-Path $coverageRoot "UDLODPreprocessor.cobertura.xml"
